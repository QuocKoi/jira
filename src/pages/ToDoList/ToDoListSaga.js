...
import { ADD_TASK_API, DELETE_TASK_API, DONE_TASK_API, GET_TASKLIST_API, UNDO_TASK_API } from '../../redux/types/ToDoListType';
export default function ToDoListSaga() {
    ...
    let getTaskList = () => {
        dispatch({ type: GET_TASKLIST_API })
    }
    ...
    let addTask = () => {
        ...
        if (index == -1) {
            dispatch({type:ADD_TASK_API,taskName:value.taskName})
        } else {
            return;
        }
    }
    let deleTask = (name) => {
        dispatch({type:DELETE_TASK_API,name})
    }
    let doneTask = (name) => {
        dispatch({type:DONE_TASK_API,name})
    }
    let undoTask = (name) => {
        dispatch({type:UNDO_TASK_API,name})
    }
    return (
        <div className="card">
